...
import { LOCK, VISUAL, formatKeyMode } from "./terminal-copy"
import type { AppElements, AudioSnapshot, TapStats } from "./types"

type EqState = "standby" | "active" | "scanning"

interface KeyScanInput {
  key: string
  mode?: string
...
let els = {} as AppElements
let eqState: EqState = "standby"
let tapSpikeStrength = 0
let tapSpikeUntil = 0
let standbyFrameId: number | null = null
let micListening = false
let gridSlots: (number | null)[] = Array(GRID_SLOTS).fill(null)

export function initAsciiVisuals(elements: AppElements): void {
  els = elements
...
export function setEqState(state: EqState): void {
  eqState = state
  if (els.asciiEqPanel) {
...
export function flashTapSpike(): void {
  tapSpikeStrength = 1
...
export function updateAsciiEqFromMic(snapshot: AudioSnapshot | null, inputLevel: number): void {
...
export function deriveTapLockPercent(stats: TapStats): number {
...
export function updateSignalLock(percent: number): void {
  renderMeter(els.signalLockBar, els.signalLockValue, clamp(Math.round(percent), 0, 95))
}

export function updateRhythmGrid(tapCount: number): void {
  if (!tapCount) return
...
export function updateKeyScan(keyResult: KeyScanInput | null, listening: boolean): void {
...
export function resetTapVisuals(): void {
  tapSpikeStrength = 0
  tapSpikeUntil = 0
...
function frequencyToBands(frequencyData: Uint8Array, sampleRate: number): number[] {
...
function renderBarLine(bands: number[]): string {
  return bands.map((height) => BAR_CHARS[height] ?? BAR_CHARS[0]).join(" ")
}

function renderMeter(barEl: HTMLElement | null, valueEl: HTMLElement | null, percent: number): void {
...
function renderRhythmGrid(): void {
