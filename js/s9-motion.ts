...
let micScanTween: gsap.core.Tween | null = null
let keyScanTween: gsap.core.Tween | null = null
...
export function initS9Motion(): void {
  if (initialized) return
  initialized = true

  if (!canAnimate()) return

  const shell = document.querySelector<HTMLElement>(".s9-app-enter")
...
export function animateStartupReveal(): void {
  if (!canAnimate()) return
...
export function animateAsciiLogoFlicker(): void {
  if (!canAnimate()) return
...
export function animateTapPulse(): void {
  if (!canAnimate()) return
...
export function animateSignalLock(target?: Element | null): void {
  if (!canAnimate()) return
...
export function animateAsciiEq(target?: Element | null): void {
  if (!canAnimate()) return
...
export function animateMicScan(isActive: boolean): void {
  if (micScanTween) {
    micScanTween.kill()
    micScanTween = null
  }
...
export function animateKeyScan(target: Element | null | undefined, state: string): void {
  if (!canAnimate()) return
...
function bindButtonPressFeedback(): void {
  document.querySelectorAll<HTMLButtonElement>(".s9-broadcast__btn").forEach((button) => {
...
export function killAllMotion(): void {
  if (micScanTween) {
    micScanTween.kill()
    micScanTween = null
  }
